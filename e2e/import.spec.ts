...
test.describe("CSV Import", () => {
  ...
  test("import page loads with step 1 (Upload) visible", async ({ page }) => {
    ...
    // Step indicator shows the three wizard steps
  ...
  test("Target Account section is shown on step 1", async ({ page }) => {
    await expect(page.getByRole("heading", { name: /target account/i })).toBeVisible()
  ...
  test("advances to column mapping step after clicking Continue", async ({ page }) => {
    ...
    // Step 2: Column Mapper should be visible
    await expect(
      page.getByRole("heading", { name: /column mapping|map columns/i })
