...
async function main() {
  ...
  // Daily at 7 AM — payday loan auto-payments
  cron.schedule("0 7 * * *", async () => {
    try {
      await runPaydayPayments()
    } catch (err) {
      console.error("[cron] Unhandled error in runPaydayPayments:", err)
    }
  })
  console.log("[cron] Registered: payday-payments (daily 7 AM)")
